...
def prepare_parser():
    ...
    parser.add_argument(
        "--dim_z", type=int, default=128, help="Noise dimensionality: %(default)s)"
    )
    parser.add_argument(
...
class MultiEpochSampler(torch.utils.data.Sampler):
    ...
    def __iter__(self):
        ...
        num_epochs = int(
            np.ceil(
                (n * self.num_epochs - (self.start_itr * self.batch_size)) / float(n)
            )
        )
    ...
    **kwargs
    ...
    if use_multiepoch_sampler:
        ...
        train_loader = DataLoader(
            train_set, batch_size=batch_size, sampler=sampler, **loader_kwargs
        )
    else:
        loader_kwargs = {
        ...
        train_loader = DataLoader(
            train_set, batch_size=batch_size, shuffle=shuffle, **loader_kwargs
        )
