...
def save_image(img, path, n_per_row=1):
    with torch.no_grad():
        torchvision.utils.save_image(
            torch.from_numpy(
                img.cpu().numpy()
            ),  ##hack, to turn Distribution back to tensor
            path,
            nrow=n_per_row,
            normalize=True,
        )
...
def get_G(resolution=256):
    if resolution == 256:
        ...
        config["device"] = "cuda" if not USECPU else "cpu"
...
class FuseDreamBaseGenerator:
    def __init__(
        ...
        self.device = device if not USECPU else "cpu"
        self.G = G
    ...
    def generate_basis(self, text, init_iters=500, num_basis=5):
        ...
        with torch.no_grad():
            for i in tqdm(range(init_iters)):
                ...
                self.z_.data = torch.clamp(
                    self.z_.data.detach().clone(), min=-Z_THRES, max=Z_THRES
                )

                image_tensors = self.G(self.z_, self.G.shared(self.y_))
                image_tensors = (image_tensors + 1.0) / 2.0
                image_tensors = F.interpolate(
                    image_tensors, size=224, mode=self.interp_mode
                )
                image_tensors.sub_(mean[None, :, None, None]).div_(
                    std[None, :, None, None]
                )

                logits_per_image, logits_per_text = self.clip_model(
                    image_tensors, text_tok
                )
                logits_per_image = logits_per_image / clip_c
                if z_init is None:
    ...
    def optimize_clip_score(
        ...
        if not optimize_y:
            ...
            optimizer = torch.optim.Adam(
                [w_z, w_y, opt_y, opt_z], lr=5e-3, weight_decay=0.0
            )
        ...
        for i in tqdm(range(opt_iters)):
            ...
            if not latent_noise:
                ...
                cur_y = cur_y.sum(dim=0, keepdim=True)
                cur_z_aug = (
                    cur_z + torch.randn(cur_z.shape).to(cur_z.device) * LATENT_NOISE
                )
                cur_y_aug = (
                    cur_y + torch.randn(cur_y.shape).to(cur_y.device) * LATENT_NOISE
                )
            ...
            for j in range(image_tensors.shape[0]):
                if augment:
                    ...
                    loss = loss + NaiveSemanticLoss(
                        image_tensors[j : (j + 1)], self.clip_model, text_tok
                    )

            loss.backward()
            optimizer.step()

            opt_z.data = torch.clamp(
                opt_z.data.detach().clone(), min=-Z_THRES, max=Z_THRES
            )
